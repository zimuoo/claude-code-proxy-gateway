...
```bash
cd openai-proxy-gateway
docker build -t openai-proxy-gateway:latest .
docker run --rm -p 8080:8080 \
  -e PROVIDERS_CONFIG_PATH=/app/config/providers.yaml \
  -e OPENAI_API_KEY=xxx \
  -e ANTHROPIC_API_KEY=xxx \
  openai-proxy-gateway:latest
```
...
你提到的这组变量是正确方向：`ANTHROPIC_BASE_URL`、`ANTHROPIC_AUTH_TOKEN`、`ANTHROPIC_MODEL`、`ANTHROPIC_SMALL_FAST_MODEL`。  
本项目已经支持 `Claude Code -> /v1/messages -> Proxy -> 多 Provider` 的链路。

### 1) 先启动代理

确保代理运行在例如：`http://127.0.0.1:8080`

### 2) 在 Claude 的配置中设置（桌面/CLI 通用思路）

通常在 `~/.claude/settings.json`（部分版本可能是 `~/.claude/setting.json`）里配置环境变量：
...
- `ANTHROPIC_MODEL` / `ANTHROPIC_SMALL_FAST_MODEL` 填你希望走的目标模型名，实际去向由 `model_provider_map` 决定。
  - 失败时若配置了 `model_fallbacks`，会按回退链自动切换 provider。

### 3) 推荐映射策略

在 `providers.yaml` 里把你给 Claude 用的模型名映射到目标 provider，例如：

- `gpt-* -> openai`
- `deepseek-* -> deepseek`
- `qwen-* -> qwen`

这样 Claude 侧只改模型名即可切换底层厂商。
...
- 不同厂商的函数调用、图像、音频、Responses 事件细节仍存在差异，建议按你的真实使用场景逐步补齐。
- 若你要生产级稳定，建议加上：限流、审计日志、多租户隔离、熔断与告警。
